...
const Sidebar: React.FC<SideBarProps> = ({ artwork, styles }) => {
  const Tags = () =>
    Array.from(artwork.tags).map((tag, index) => {
      return (
        <li key={index} className="flex bg-rose rounded-full px-3 sm:px-5 py-1 sm:py-2">
          {tag}
        </li>
      );
    });
  const currIndex = artworks.indexOf(artwork);
  const prevArt: Artwork | null =
    currIndex > 0 ? artworks[currIndex - 1] : null;
  const nextArt: Artwork | null =
    currIndex < artworks.length - 1 ? artworks[currIndex + 1] : null;
  ...
  const prevButton = () => {
    if (prevArt != null) {
      return (
        <Link
          href={prevArt.key}
          className="flex items-center px-5 py-2 bg-lime text-white text-sm rounded-full"
        >
          <svg
