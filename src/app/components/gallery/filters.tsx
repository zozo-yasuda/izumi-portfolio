...
const Filters = ({
  ...
  const filterItems = filterTags.map((filter, index) => (
    <li key={index} className="uppercase tracking-18 text-green p-1 sm:p-2">
      <span
        onClick={() => handleFilterClick(filter)}
        className="relative inline-block cursor-pointer"
      >
        <div className="text-green tracking-18 relative z-10 text-xs sm:text-sm b-2 b-offwhite">
          {filter}
        </div>
        {activeFilter === filter && (
